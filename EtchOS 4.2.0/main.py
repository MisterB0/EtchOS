...
LANGUAGES = {
    "en": {
        "welcome": "Welcome to EtchOS 4.2.0",
    ...
    "de": {
        "welcome": "Willkommen bei EtchOS 4.2.0",
    ...
    "fr": {
        "welcome": "Bienvenue sur EtchOS 4.2.0",
...
def MainMenu(stdscr, config):
    ...
    # Multitasking state
    active_apps = []  # List of (app_name, subprocess_obj)
    current_app_idx = -1
    ...
    while True:
        ...
        # Window Manager mode
        if active_apps:
            stdscr.addstr(0, 0, " WINDOW MANAGER | Tab: Switch | Q: Quit App | Enter: Open | Esc: Main Menu ", curses.A_REVERSE)
            
            app_name, proc = active_apps[current_app_idx]
            box_w = max_x - 4
            box_h = max_y - 3
            draw_box(stdscr, 1, 2, box_h, box_w)
            safe_addstr(stdscr, 1, 3, f" {app_name} ", curses.A_BOLD)
            
            stdscr.addstr(max_y - 1, 2, f"Active App: {app_name} | Total: {len(active_apps)}")
            stdscr.refresh()
            
            key = stdscr.getch()
            if key == 9:  # Tab
                current_app_idx = (current_app_idx + 1) % len(active_apps)
            elif key == ord('q') or key == ord('Q'):
                proc.terminate()
                active_apps.pop(current_app_idx)
                current_app_idx = 0 if active_apps else -1
            elif key == 27:  # Esc
                for _, p in active_apps:
                    p.terminate()
                active_apps = []
                current_app_idx = -1
            elif key in [ord('\n'), ord(' ')]:
        ...
        if (key == curses.KEY_UP or key == ord('w') or key == ord('W')) and selected > 0:
            ...
            active_apps.append((app_name, proc))
            current_app_idx = len(active_apps) - 1
        elif key in [ord('x'), ord('X')]:
            log_action("shutdown_initiated", {"user": config.get("username")})
            max_y, max_x = stdscr.getmaxyx()
            box_width, box_height = 40, 5
            box_left, box_top = get_center_x(max_x, box_width), (max_y - box_height) // 2
            stdscr.clear()
            draw_box(stdscr, box_top, box_left, box_height, box_width)
        ...
        if not config.get("onboarding_done"):
            ...
            while not login_success:
                ...
                if not login_success:
                    ...
                    # We don't need to endwin and recurse because we are in a while loop
                    # but we should clear the screen for the next attempt
                    stdscr.clear()
                    continue
